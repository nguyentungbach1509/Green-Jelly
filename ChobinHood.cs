...
/*
Handle movement and states of enemies Chobinhood
*/
public class ChobinHood : Enemy
{
        ...
        if(distance <= chasingRange && distance > attackRange && !playerMovement.playerBeKilled) {
            ...
            if(currentState != EnemyStates.enemystunning) {
                if(duringSpecialCooldown == false) {
                    ...
                    if(currentState != EnemyStates.enemystunning && duringCooldown == true) {
    ...
    public void spawnSpecialArrow() {
        ((Instantiate(am, transform.position, Quaternion.identity)).GetComponent<ArrowManagement>()).arrowNo = 3;
    }
