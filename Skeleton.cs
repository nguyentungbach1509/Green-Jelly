...
public class Skeleton : Enemy
{
    ...
    private bool duringDefCooldown = false;
    ...
    void Update() {
        ...
        calculateDefenceCooldown();
        animator.SetBool("Hurt", (currentState == EnemyStates.enemystunning));
        animator.SetBool("ShieldTouching", (currentState == EnemyStates.enemydef));
    ...
    public virtual void DistanceChecking() {
        ...
        if(distance <= chasingRange && distance > attackRange && !playerMovement.playerBeKilled) {
            ...
            if(currentState != EnemyStates.enemystunning && currentState != EnemyStates.enemydef) {
                if(duringSpecialCooldown == false) {

                        duringSpecialCooldown = true;
                        duringCooldown = true;
                        duringDefCooldown = true;
                        attackTimer = 1;
                        defTimer = 1;
                        specialTimer = 1;
                        SpecialAttackPlayer(); 
                    ...
                    if(currentState != EnemyStates.enemystunning && currentState != EnemyStates.enemydef && duringCooldown == false) {
                    ...
                    if(currentState != EnemyStates.enemystunning && currentState != EnemyStates.enemyattack && duringDefCooldown == false) {
                        duringDefCooldown = true;
                        defTimer= 1;
                        prepareDef = true;
                        DefenceToPlayer();
    ...
    private void DefenceToPlayer() {
        StateChanging(EnemyStates.enemypreparedef);
        Vector3 temp = Vector3.MoveTowards(transform.position, playerTransform.position, enemySpeed * Time.deltaTime);
        AnimationMovement(temp - transform.position);
    ...
    private void calculateDefenceCooldown() {
         if(duringDefCooldown) {

            defTimer -= 1 / defCoolDown * Time.deltaTime;

            if(defTimer <= 0) {
                duringDefCooldown = false;
                defTimer = 0;
            }
        }
    }
