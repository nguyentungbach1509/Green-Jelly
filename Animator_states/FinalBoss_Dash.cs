...
public class FinalBoss_Dash : StateMachineBehaviour
{
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        if(playerTransform != null) {
            ...
            if(Vector2.Distance(playerTransform.position, rb.position) > minRange) {

                animator.SetTrigger("StopDash");

            }
            else {
                if(finalBoss.enemyTouchingEnvironment) {
                    animator.SetTrigger("Skill2_Disappear");
                }
                else {
    ...
    {
        animator.ResetTrigger("StopDash");
        animator.ResetTrigger("Skill2_Disappear");
    }
}
