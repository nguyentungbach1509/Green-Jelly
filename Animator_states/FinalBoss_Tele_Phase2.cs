...
public class FinalBoss_Tele_Phase2 : StateMachineBehaviour
{
    ...
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        if(GameObject.FindGameObjectWithTag("Player") != null) {
            ...
            if(target.x <= -75f || target.x >= -46f 
            || target.y >= 19f || target.y <= 3f) {
                aline = new Ray2D(rb.transform.position, (enemyDirection * -1f));
                target = aline.GetPoint(Vector2.Distance(playerTransform.position, rb.position) + 5f);
